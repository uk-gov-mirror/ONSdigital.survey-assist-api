...
- **Suggestions**: `POST /v1/survey-assist/suggestions` returns typeahead suggestions from the SAYT service (`type: "sic"`).
...
- **`SAYT_SERVICE_AUTH_ENABLED`**: Optional, default is set to True. Set as False when running the API and SAYT service locally or the SAYT service is a side-car deployment alongside the API in Cloud Run
